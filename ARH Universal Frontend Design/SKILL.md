---
name: arh-frontend-design-hld
description: Use when the user asks to design, review, redesign, implement, or quality-check frontend interfaces, UI flows, dashboards, landing pages, prototypes, or design-system decisions; applies context-adaptive design direction, token-efficient design editing, accessibility, performance, responsive-state coverage, and practical validation scripts.
---
...
Use bundled resources only when the task needs them:

- `references/frontend-design-gold/resources/frontend-evaluation-rubric.md` — deeper review rubric for frontend audits.
- `references/frontend-design-gold/resources/reference-anchors.md` — source anchors and standards to validate design claims.
- `references/arh-frontend-design-skill-HLD-v1.md` — preserved HLD/source version for review history.
- `references/frontend-design-editing-layer.md` — maintenance brief for the token-efficient design editing layer and future `set-*` scripts.
- `assets/shadcn-components.tar.gz` — vendored shadcn/ui component starter archive used by `scripts/init-artifact.sh`.

## Scripts

Run scripts from a generated frontend project root unless noted otherwise:

- `scripts/init-artifact.sh <project-name>` — create a React + Vite + Tailwind + shadcn/ui starter project from this skill package.
- `scripts/design.config.json` — source config for routine visual tokens copied into generated projects.
- `scripts/generate-tailwind-config.sh` — regenerate Tailwind/CSS design output from `design.config.json`.
- `scripts/set-spacing-scale.sh <compact|comfortable|spacious>` — mutate `designTokens.spacingScale`, regenerate design output, and return compact JSON.
- `scripts/generate-design-tokens.sh` — extract Tailwind design tokens to `design-tokens.json` and `design-tokens.css`.
- `scripts/setup-storybook.sh` — add Storybook with accessibility, token, theme, and viewport support.
- `scripts/audit-accessibility.sh` — start the dev server and run axe-core accessibility checks through Playwright.
- `scripts/performance-budget.sh` — build with bundle stats and enforce a 500 KB first-load budget signal.
- `scripts/bundle-artifact.sh` — build a single-file `bundle.html` artifact with design validation checks.
- `scripts/archive-project.sh` — archive the current project while excluding generated dependency/build folders.

## Token-efficient design editing

For generated projects that contain `design.config.json`, prefer the design editing layer before manual Tailwind/CSS edits.

Use token scripts for high-frequency, low-risk visual changes:
...
Do manual component/page work for changes that are not token-driven:

- New page layout.
- Component hierarchy.
- Copy and information architecture.
- Domain-specific states.
- Custom interaction or animation choreography.
...
3. Run the matching script with `--dry-run` first when available.
4. Run the script for real.
5. Let `scripts/generate-tailwind-config.sh` produce derived files.
6. Validate with the smallest useful command.
7. Report the JSON result, files touched, validation result, and any remaining visual risk.

If the needed `set-*` script does not exist and the edit is generally reusable, add the script to the skill package and `init-artifact.sh` rather than doing a one-off manual mutation. Do not advertise a package script until the matching file is actually copied into generated projects.

Every future `scripts/set-*.sh` should:

- Accept one focused value or preset.
- Validate allowed values before writing.
- Support `--dry-run`.
- Mutate `design.config.json` as the source of truth.
- Call `scripts/generate-tailwind-config.sh` or `./scripts/generate-tailwind-config.sh`.
- Return compact JSON with `status`, `changed`, `summary`, `evidence`, and `next_action_hint`.
- Avoid long prose output.

Prefer structured JSON mutation with Node, `jq`, or `jaq`. Avoid `sed` for JSON except as a temporary fallback with an explicit warning.

Generator rules:

- `scripts/generate-tailwind-config.sh` is the only writer for derived design files.
- Prefer marked generated blocks over whole-file replacement when preserving user CSS matters.
- Do not assume optional app files exist; report missing targets in JSON.
- Do not leave `.bak` files in the project root; use `_history/` or `.design-history/` for backups.

## Validation contract

Before relying on this skill package after edits:

1. Run `arh-skill-qa check <skill-folder> --json`.
2. Run shell syntax checks for every file in `scripts/`.
3. For project-generation changes, smoke-test `scripts/init-artifact.sh <temp-project>` in a disposable folder.
4. For design-token script changes, run the relevant `scripts/set-*.sh --dry-run` and one real mutation in a disposable generated project.
5. For generated projects, run `pnpm build`, `scripts/audit-accessibility.sh`, and `scripts/performance-budget.sh` where dependencies and browser tooling are available.
...
- If the project has `design.config.json`, use token scripts for routine visual edits before manually editing Tailwind, CSS, or component files.
...
### If handing over design-editing work

Use a compact handover block:
...
- `references/frontend-design-gold/resources/frontend-evaluation-rubric.md`
- `references/frontend-design-gold/resources/reference-anchors.md`
